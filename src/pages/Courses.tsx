...
import { mockCourses as initialCourses } from "@/data/mockCourseStore";
import { Course } from "@/types/dcampus";
...
const Courses = () => {
  ...
  const handleDelete = () => {
    if (deleteItem) {
      ...
      toast({ title: "Curso eliminado", description: `"${deleteItem.title}" fue eliminado.` });
      setDeleteItem(null);
    }
  };
  ...
  return (
    <DashboardLayout title="Cursos">
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          <motion.div key={course.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            ...
            <div className="p-5">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${course.status === "active" ? "bg-primary/10 text-primary" : "bg-accent/10 text-accent-foreground"}`}>
              ...
              <div className="flex items-center gap-2 pt-3 border-t border-border">
                ...
                <button onClick={() => setDeleteItem(course)}
        ...
        {filtered.length === 0 && (
          <div className="col-span-full glass-card rounded-xl p-12 text-center text-muted-foreground text-sm">No se encontraron cursos.</div>
        )}
      </div>

      <DeleteDialog open={!!deleteItem} onClose={() => setDeleteItem(null)} onConfirm={handleDelete}
        title="Eliminar curso" description={`¿Estás seguro de eliminar "${deleteItem?.title}"? Esta acción no se puede deshacer.`} />
    </DashboardLayout>
  );
};
